...
@section('title', isset($selectedCategory) && isset($categories[$selectedCategory]) ? $categories[$selectedCategory]['name'] . ' - Digital Store' : 'All Categories - Digital Store')
...
<nav aria-label="breadcrumb" class="breadcrumb-section">
  <div class="container">
    <ol class="breadcrumb">
      ...
      @if($isCategorySelected)
        <li class="breadcrumb-item active" aria-current="page" style="color:white;">{{ $activeCategoryName }}</li>
      @else
...
<section class="section-padding category-layout-section">
  <div class="container">
    <div class="row g-4">
      ...
      {{-- SIDEBAR COLUMN (Left: Categories List) --}}
      <div class="col-lg-3">
        <div class="sidebar-panel p-4">
          ...
          <div class="list-group list-group-sidebar gap-2">
            ...
            <a href="{{ route('categories') }}" 
               class="list-group-item list-group-item-action d-flex align-items-center justify-content-between py-2.5 px-3 {{ !$isCategorySelected ? 'active' : '' }}">
              <span><i class="bi bi-collection me-2"></i> All Categories</span>
              <span class="badge rounded-pill bg-dark-pill">{{ count(Arr::collapse($products)) }}</span>
            </a>
            
            {{-- Category Specific Options --}}
            @foreach($categories as $slug => $cat)
              <a href="{{ route('categories', $slug) }}" 
                 class="list-group-item list-group-item-action d-flex align-items-center justify-content-between py-2.5 px-3 {{ $selectedKey === $slug ? 'active' : '' }}">
                <span><i class="bi {{ $cat['icon'] }} me-2"></i> {{ $cat['name'] }}</span>
                <span class="badge rounded-pill bg-dark-pill">{{ $cat['count'] }}</span>
              </a>
            @endforeach
          </div>
        </div>
      ...
      {{-- CATEGORY CONTENT COLUMN (Right: Header, Search, Sorting, Grid, Pagination) --}}
      <div class="col-lg-9">
        
        {{-- Category Header and Count --}}
        <div class="category-header-wrap mb-4" style="--cat-accent-color: {{ $activeCategoryColor }}; --cat-accent-glow: {{ $activeCategoryGlow }};">
          <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center border-bottom border-secondary-subtle pb-3 mb-2">
            <div>
              <h2 class="category-main-title mb-1">{{ $activeCategoryName }}</h2>
              <p class="product-count-label text-secondary small mb-0" id="total-count-text">
                <span id="filtered-count">{{ $productCount }}</span> Products Found
              </p>
            </div>
            @if($isCategorySelected)
              <span class="badge-cat-pill d-none d-sm-inline-block" style="background: {{ $activeCategoryGlow }}; color: {{ $activeCategoryColor }};">
        ...
        {{-- Interactive Controls: Search and Sort --}}
        <div class="controls-card p-3 mb-4">
          <div class="row g-3">
            {{-- Search Bar --}}
            <div class="col-md-8">
              <div class="search-input-wrapper">
                <input type="text" id="live-product-search" class="form-control form-control-custom" placeholder="Search in {{ $activeCategoryName }}...">
        ...
        <div class="products-grid-container mb-5" id="products-list-wrapper">
          <div class="row g-4" id="products-list-row">
            @forelse($filteredProductsList as $idx => $prod)
              <div class="col-12 col-md-6 product-showcase-card-wrapper" 
                ...
                <div class="product-premium-card h-100 d-flex flex-column">
                  <div class="product-card-top p-3 d-flex align-items-start justify-content-between">
                    <div class="product-card-icon {{ $prod['class'] }}" style="background: {{ $activeCategoryGlow }}; color: {{ $activeCategoryColor }};">
                  ...
                  <div class="product-card-body px-4 pb-3 flex-grow-1">
                    <h4 class="product-card-title mb-2">{{ $prod['title'] }}</h4>
                    <p class="product-card-desc text-secondary small mb-0">{{ $prod['desc'] }}</p>
                  </div>
                  <div class="product-card-footer p-4 d-flex align-items-center justify-content-between mt-auto">
                    <div class="product-card-price">${{ number_format($prod['price'], 2) }}</div>
